...
(function ($) {
	...
	var translations = {
			...
			no_matching_close: function (lineNro, block) {
				return (
					block + ' lohkoa riviltä ' + lineNro + ' ei ole päätetty.'
				);
			},
  ...
  */
	...
	LineBasedGrader.prototype.grade = function (options, elementId) {
		...
		var lisStudentCodeLineObjects = studentCodeLineObjects.filter(
			function (lineObject) {
		...
		$.each(inv, function (_index, lineObjectIndex) {
			...
			!options.skipHighlight &&
				lisStudentCodeLineObjects[
					lineObjectIndex
				].markIncorrectPosition();
			incorrectLines.push(
				lisStudentCodeLineObjects[lineObjectIndex].orig
			);
		});
		...
		trashContext
			...
			.on('click', '.jsparson-toggle', function () {
				var $this = $(this),
					...
					newVal =
						choices[(choices.indexOf(curVal) + 1) % choices.length],
	...
	var ParsonsCodeline = function (codestring, widget) {
		...
		if (codestring) {
			...
			this.code = codestring
				...
				.replace(/\\n/g, '\n');
			this.indent =
				codestring.length - codestring.replace(/^\s+/, '').length;
		}
	};
	...
	ParsonsCodeline.prototype._addToggles = function () {
		var toggleRegexp = new RegExp(
			'\\$\\$toggle(' +
				this.widget.options.toggleSeparator +
				'.*?)?\\$\\$',
			'g'
		);
	...
	ParsonsWidget.prototype._getToggleStates = function () {
		var context = $(
			...
			toggleStates = {};
		$('#' + this.options.sortableId + ' .jsparson-toggle').each(
			function () {
		...
		if (this.options.trashId) {
			toggleStates.input = [];
			$('#' + this.options.trashId + ' .jsparson-toggle').each(
				function () {
	...
	ParsonsWidget.prototype.clearFeedback = function () {
		if (this.feedback_exists) {
			$('#ul-' + this.options.sortableId).removeClass(
				'incorrect correct'
			);
	...
	ParsonsWidget.prototype.codeLinesToHTML = function (
		...
		return (
			'<ul id="ul-' + destinationID + '">' + lineHTML.join('') + '</ul>'
		);
	};
	...
	ParsonsWidget.prototype.createHTMLFromLists = function (
		...
		var sortable = $('#ul-' + this.options.sortableId).sortable({
			...
			stop: function (event, ui) {
				...
				that.addLogEntry(
					{ type: 'moveOutput', target: ui.item[0].id },
					true
				);
			},
			receive: function (event, ui) {
				...
				that.updateHTMLIndent(ui.item[0].id);
				that.addLogEntry(
					{ type: 'addOutput', target: ui.item[0].id },
					true
				);
			},
