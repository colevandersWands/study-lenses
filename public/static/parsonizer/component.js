	...
	async connectedCallback() {
		...
		if (this.blockComments) {
			...
			for (const blockComment of this.blockComments) {
				...
				const toCollapse = blockComment.match(
				...
				if (toCollapse) {
					...
					blockCommentContainer.appendChild(
	...
	const studyWith = new Proxy(
		{
			...
			loupe(code) {
				...
				const loupeURL =
					'http://latentflip.com/loupe/?code=' + encoded + '!!!';
				window.open(loupeURL, '_blank');
			},
			promisees(code) {
				const encoded = encodeURIComponent(code).replace(/%20/g, '+');
				const URL =
					'https://bevacqua.github.io/promisees/#code=' + encoded;
				window.open(URL, '_blank');
			},
			esprima(code) {
				const encoded = encodeURIComponent(code);
				const URL =
					'https://esprima.org/demo/parse.html?code=' + encoded;
				window.open(URL, '_blank');
			},
			flowchart(code) {
				const lenseConfig = {
