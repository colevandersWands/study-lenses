...
const ceskAdviceSignature =
	(adviceName, userAdvice, userPointcut, history, aran) =>
	(...args) => {
		const node = state.aran.nodes[serial];
		// do your cesky thing, use history
		if (
			typeof userAdvice === 'function' &&
			userPointcut(adviceName, node)
		) {
...
ceskAdvice.arrival =
	(advice) => (callee, newtarget, self, arguments, serial) => {
...
ceskAdvice.apply = (advice) => (value1, value2, values, serial) => {
	...
	if (ceskState.scopeof.has(value1)) {
		const shadows = values.length
			? ceskState.stack.splice(-values.length)
			: [];
		const shadow = ceskState.stack.pop();
