...
- What type of control flow is it?`,
	...
	},
	{
		...
		template: ({ nodes }) => {
			...
			const randomIf =
				validNodes[(validNodes.length * Math.random()) | 0];
...
-  What type(s) are assigned to it in the program?`,
			hints: [
				'the "variables" lens can help to find all the assignments',
			],
		}),
		levels: [1],
	...
	},
	{
		...
		template: ({ filtered }) => {
			const randomLiteral =
				filtered[(filtered.length * Math.random()) | 0];
			const typeName =
...
- Is it important later on in the program?`,
	...
	// === operators ===
	{
		...
		template: ({ nodes }) => {
			const operators = [
				...new Set(
					filters.operatorNodes({ nodes }).map((n) => n.operator)
				),
			];
			const randomOperator =
				operators[(operators.length * Math.random()) | 0];
			return {
				question: `How many times is the '${randomOperator}' operator used in this program?`,
				hints: [
					'you can use control-f in the editor to search the code',
				],
			};
		},
...
- Can you think of a better name?`,
	...
	},
	{
		template: ({ nodes }) => {
			...
			const node =
				uniqueIdentifiers[
					(uniqueIdentifiers.length * Math.random()) | 0
				];
...
- What is it used for in the program?`;
	...
	},
	{
		...
		template: ({ nodes }) => {
			...
			const declaration =
				nestedDeclarations[
					(nestedDeclarations.length * Math.random()) | 0
				];
...
- How many of them are declared in this block?`,
	...
	},
	{
		...
		template: ({ nodes }) => {
			...
			const assignment =
				nestedAssignments[
					(nestedAssignments.length * Math.random()) | 0
				];
...
- What are they asked to input?`,
...

	{
		template: ({ code }) => {
			...
			const randomLine =
				codeLines[(codeLines.length * Math.random()) | 0];
			return {
